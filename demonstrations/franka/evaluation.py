...
success_rate_force = np.load(os.getcwd()+"/data/training_results/f_pforce_franka.npy")
success_rate_noforce = np.load(os.getcwd()+"/data/training_results/n_pforce_franka.npy")
success_rate_panda = np.load(os.getcwd()+"/data/training_results/p_pforce_franka.npy")
f_t_force = np.load(os.getcwd()+"/data/training_results/force_force.npy")
f_t_noforce = np.load(os.getcwd()+"/data/training_results/noforce_force.npy")
f_t_panda = np.load(os.getcwd()+"/data/training_results/panda_force.npy")
...
for demonstrator in name_list:
    for i in range(5):
        f_no_force.append(np.load(os.getcwd()+"/data/no_force/finger_force_"+demonstrator+"_"+str(i)+".npy"))
        f_force.append(np.load(os.getcwd()+"/data/force/finger_force_"+demonstrator+"_"+str(i)+".npy"))
        f_panda.append(np.load(os.getcwd()+"/data/panda/finger_force_"+demonstrator+"_"+str(i)+".npy")/1.5)
...
plt.hlines(0, 0, 50,color="orange",linestyles = '-')

y1=s_f_noforce.rolling(150).mean()-0.1*s_f_noforce.rolling(150).std()
y2=s_f_noforce.rolling(150).mean()+0.1*s_f_noforce.rolling(150).std()
for i in range(len(y1)):
    if y1[i] < 0:
        y1[i] = 0
plt.fill_between(x_new, y1, y2, alpha=0.3,color=colors[0])

y1=s_f_force.rolling(50).mean()-s_f_force.rolling(50).std()
y2=s_f_force.rolling(50).mean()+s_f_force.rolling(50).std()
for i in range(len(y1)):
    if y1[i] < 0:
        y1[i] = 0
...
train_noforce, = plt.plot(s_f_noforce.rolling(150).mean(),color=colors[0])
train_force, = plt.plot(s_f_force.rolling(50).mean(),color=colors[1])
