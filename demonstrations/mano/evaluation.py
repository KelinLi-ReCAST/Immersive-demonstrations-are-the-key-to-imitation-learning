...
success_rate_force = np.load(os.getcwd()+"/data/training_results/f_pforce_mano.npy")
success_rate_noforce = np.load(os.getcwd()+"/data/training_results/n_pforce_mano.npy")
success_rate_panda = np.load(os.getcwd()+"/data/training_results/p_pforce_mano.npy")
...
for demonstrator in name_list:
    for i in range(5):
        ...
        f_panda.append(np.load(os.getcwd()+"/data/panda/finger_force_"+demonstrator+"_"+str(i)+".npy")/2)
...
y1=s_f_force.rolling(200).mean()-0.2*s_f_force.rolling(200).std()
y2=s_f_force.rolling(200).mean()+0.2*s_f_force.rolling(200).std()
for i in range(len(y1)):
    if y1[i] < 0:
        y1[i] = 0
...
train_force, = plt.plot(s_f_force.rolling(200).mean(),color=colors[1])
