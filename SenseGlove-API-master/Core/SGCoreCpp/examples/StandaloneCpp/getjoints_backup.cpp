...
//---------------------------------------------------------------------------
...

	{
                ...
                for (int i = 0; handPose.ToString()[i] != '\0'; ++i) //当a数组元素不为结束符时.遍历字符串a.
     			{
         			if (handPose.ToString()[i] >= '0'&& handPose.ToString()[i] <= '9') //如果是数字.
         			{
					    if (handPose.ToString()[i+1] >= '0'&& handPose.ToString()[i+1] <= '9')
						{
							...
							{
								aa[0] = handPose.ToString()[i];
             			    	cnt_int = (handPose.ToString()[i] - '0')*100 + (handPose.ToString()[i+1] - '0')*10 + handPose.ToString()[i+2] - '0';
		...
		//for (int i = 0; i<1; ++i)
		//{
			//sprintf(temp, "%d", joints[i]);
			//charTowchar(chr, wchar, sizeof(wchar_t)* 4);
		//}
