...
eval_force = np.sort(np.load(DATA_DIR+"/test_force_mano.npy")*4)
force_mean = np.zeros(25)
eval_noforce = np.sort(np.load(DATA_DIR+"/test_noforce_mano.npy")*4)
...
for f in [f_force,f_no_force,f_panda]:
    ...
    for i in range(len(f)):
        f[i] = np.delete(f[i], range(3,29), axis=1)
        fsum = 0
        count = 0
