...
DATA_DIR = os.path.join(REPO_DIR, 'data', 'franka')
VIDEO_DIR = os.path.join(REPO_DIR, 'media', 'videos', 'franka')
import numpy as np
...
success_rate_force = np.load(DATA_DIR+"/training_results/f_pforce_franka.npy")
success_rate_noforce = np.load(DATA_DIR+"/training_results/n_pforce_franka.npy")
success_rate_panda = np.load(DATA_DIR+"/training_results/p_pforce_franka.npy")
...
for demonstrator in name_list:
    for i in range(5):
        ...
        f_panda.append(np.load(DATA_DIR+"/panda/finger_force_"+demonstrator+"_"+str(i)+".npy")/1.5)
