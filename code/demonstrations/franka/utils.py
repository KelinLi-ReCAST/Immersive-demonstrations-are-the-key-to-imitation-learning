...
def hand_calibration(glove,demonstrator):
  ...
  file = open(DATA_DIR+"/ax.txt",'w')
  file.write(str([cali,cali_min]))
  file.close()
  npsave = np.concatenate((cali,cali_min)).copy()
  for gripper in ["franka","RUTH","mano"]:
    np.save(os.path.join(REPO_DIR,"data",gripper,"glove_calibration",demonstrator),npsave)
...
def hand_boundary(joints, glove_scale, cali_min):
    ...
    for i in range(21):
        if i in bend_list:
            ...
            joints[i] = round(-0.04*(joints[i]-cali_min[i])/glove_scale[i],4)
            if joints[i] < -0.04:
                joints[i] = -0.04
        if i in finger_list:
...
def tracker(v,kp,ki,x_c,y_c,z_c,manoUid,x_rot,y_rot,z_rot):
    ...
    x_rot = x_rot + ang_vel_tracker[0]*3.14/180*1/10
    y_rot = y_rot + ang_vel_tracker[1]*3.14/180*1/10
    z_rot = z_rot + ang_vel_tracker[2]*3.14/180*1/10
